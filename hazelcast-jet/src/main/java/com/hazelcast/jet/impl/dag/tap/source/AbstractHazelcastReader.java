...
public abstract class AbstractHazelcastReader<V> implements DataReader {
    ...
    @Override
    public void close() {
        closeFromCurrentThread();
    }

    private void closeFromCurrentThread() {
    ...
    public void open() {
        ...
        if (readFromPartitionThread()) {
            openFromPartitionThread();
        } else {
            openFromCurrentThread();
        }
    }

    private void openFromCurrentThread() {
    ...
    private void openFromPartitionThread() {
        this.future.reset();
    ...
    private Object[] readFromCurrentThread() {
        ...
        } while ((hashNext = hasNext()) && (idx < this.chunkSize));
